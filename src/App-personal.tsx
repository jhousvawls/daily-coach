...
function PersonalApp({ teamMember = undefined, teamMemberData = null, isTeamMode = false }: PersonalAppProps) {
  ...
  const [goals, setGoals] = useState<Goals>(() => {
    if (isTeamMode && teamMemberData) {
      // Convert team goals to regular goals format
      return {
        personal: teamMemberData.goals.personal.map(goal => ({
          id: goal.id,
          text: goal.text,
        ...
        professional: teamMemberData.goals.professional.map(goal => ({
          id: goal.id,
          text: goal.text,
          progress: goal.progress,
          subtasks: goal.subtasks.map(st => ({
  ...
  const [tinyGoals, setTinyGoals] = useState<TinyGoal[]>(() => {
    if (isTeamMode && teamMemberData) {
      return teamMemberData.tinyGoals;
    }
    return storage.getTinyGoals();
  });
  const [dailyTasks, setDailyTasks] = useState<DailyTasks>(() => {
    if (isTeamMode && teamMemberData) {
      return teamMemberData.dailyTasks;
    }
  ...
  // Persist data changes - use team storage when in team mode
  useEffect(() => {
    if (isTeamMode && teamMember) {
      // Convert goals back to team format and save to team storage
      const teamGoals = {
        personal: goals.personal.map(goal => ({
          id: goal.id,
          text: goal.text,
          ...
          createdAt: goal.createdAt,
          updatedAt: goal.updatedAt,
          assignedBy: 'self', // Self-created goals
        ...
        professional: goals.professional.map(goal => ({
          id: goal.id,
          text: goal.text,
          description: undefined,
          category: goal.category,
